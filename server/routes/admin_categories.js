...
module.exports = app => {
  ...
  app.post("/admin/categories", async (req, res, next) => {
		...
		if (return_message.length > 0) { // hvis arrayet return_message's længde er større end 0
		   ...
		   if (result.affectedRows > 0) { // hvis der er mere end 1 række der er blevet påvirket i databasen (altså at den har modtaget noget) så kør if statement koden
			  // console.log(affectedRows);
			  res.redirect('/admin/categories'); // omdirriger til /contact siden men med ?success på.
		   }
		   else {
			  res.render('admin_categories', {
				 "categoriesData":categories,
